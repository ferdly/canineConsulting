...
let errorString = '';
// error = true;
// errorString = 'Test of Error Message';
...
let forBreakAfter = 2;
if (forBreakAfter < 0) {
    throw new Error('forBreakAfter is Negative: die()');
}

let ioSuppresCLb4 = -1;
let iaSuppresCLb4 = -1;
let iaoSuppresCLb4 = -1;
let nullSuppresCLb4 = 0;
let ioCL = forBreakAfter > 777 ? true : false;
let iaCL = forBreakAfter > 0 ? true : false;
let nullCL = forBreakAfter > 777 ? true : false;
let assignCL = forBreakAfter > 777 ? true : false;
let iIndexCL = forBreakAfter > 14 ? true : false;
let defaultCL = assignCL;//redundant so typos hell not an issue

for (let forIndex = 0; forIndex < assinmentPathArray.length; forIndex++) {
    element = assinmentPathArray[forIndex];
    clElementPrev = clElement;
    ...
    clElementNext = "Nname:'" + elementNext[0] + "',path:'" + elementNext[1] + "', index:'" + elementNext[2] + "', ruMePath:'" + elementNext[3] + "', ruMeCode:'" + elementNext[4] + "'";
    name = element[0];
    path = element[1];
    index = element[2];
    ruMePath = element[3];
    ...
    console.log(uiKeyUnChecked);
    if (false) {
        throw new Error('forceHalt');;
    }
    ...
    switch (true) {
        case ruMePath.length === 0:
            ...
            // processedPathArray.push(element);
            break;
        case uiKeyUnChecked:
            if (forIndex === forBreakAfter) {
                console.warn("forIndex === " + forIndex + " || uiKeyUnChecked || " + clElement)
            } else {
                deleteMe = "Reggie (Sheldon)";
                console.warn("uiKeyUnChecked[" + forIndex + "]: " + uiKeyUnChecked);
            }
        ...
        case index === 'i':
            if (forIndex === forBreakAfter) {
                console.warn("forIndex === " + forIndex + " || index === 'i' || " + clElement)
            } else {
                deleteMe = "Reggie (Sheldon)";
                console.warn("iIndexCL[" + forIndex + "]: " + iIndexCL);
            }
            element.unshift('INSTANTIATE_OBJECT');
            element.unshift(forIndex);
            processedPathArray.push(element.toString());
        ...
        case name === 'INSTANTIATE_OBJECT':
            ...
            eval(attributeChain);
            if (forIndex === forBreakAfter) {
                console.warn("forIndex === " + forIndex + " || name === 'INSTANTIATE_OBJECT' || " + clElement)
            } else {
                deleteMe = "Reggie (Sheldon)";
                console.warn("ioCL[" + forIndex + "]: " + ioCL);
            }
            element.unshift('INSTANTIATE_OBJECT');
            element.unshift(forIndex);
            processedPathArray.push(element.toString());
        ...
        case name === 'INSTANTIATE_ARRAY':
            ...
            eval(attributeChain);
            if (forIndex === forBreakAfter) {
                console.warn("forIndex === " + forIndex + " || name === 'INSTANTIATE_ARRAY' || " + clElement)
            } else {
                deleteMe = "Reggie (Sheldon)";
                console.warn("iaCL[" + forIndex + "]: " + iaCL);
            }
            element.unshift('INSTANTIATE_OBJECT');
            element.unshift(forIndex);
            processedPathArray.push(element.toString());
        ...
        case ruMePath === 'NNULL':
            if (forIndex === forBreakAfter) {
                console.warn("forIndex === " + forIndex + " || ruMePath === 'NNULL' || " + clElement)
            } else {
                deleteMe = "Reggie (Sheldon)";
            }
            element.unshift('NNULL');
            element.unshift(forIndex);
            processedPathArray.push(element.toString());
            break;
        ...
        default:
            ...
            eval(evalString);
            if (forIndex === forBreakAfter) {
                console.warn("forIndex === " + forIndex + " || ruMePath === " + ruMePath + " || " + clElement)
            } else {
                deleteMe = "Reggie (Sheldon)";
                console.warn("defualtCL[" + forIndex + "]: " + nullCL);
            }
            element.unshift('NNULL');
            element.unshift(forIndex);
            processedPathArray.push(element.toString());
    ...
    if (forIndex === forBreakAfter) {
        fullConsoleWarn = '<forBreakAfter>\n';
        fullConsoleWarn += 'forIndex === ' + forBreakAfter + ' || Current === ' + clElement;
        if (forIndex > 0) {
            fullConsoleWarn += '\nforIndex === ' + (forBreakAfter -1) + ' || Previous === ' + clElementPrev + '';
        }
        fullConsoleWarn += '\nforIndex === ' + (forBreakAfter + 1) + ' || Next === ' + clElementNext + '\n</forBreakAfter>';
        fullConsoleWarn += '\n</forBreakAfter>';
        console.warn(fullConsoleWarn);
        break;
    }
}
...
console.warn("<insertWixContactObject>\n" + insertWixContactJSON + "\n</insertWixContactObject>");
console.log(processedPathArray);
if (error) {
